...
contract RaffleTest is Test {
    ...
    function setUp() public {
        ...
        sels[3] = RaffleFacet.setSnapshotBlock.selector;
    ...
    function _create() internal returns (uint256 id) {
        vm.prank(owner_);
        id = r.createRaffle("The first 1/1", "ipfs://placeholder", uint64(block.number - 10), uint64(block.number + 20), 1, _weights());
    }
    ...
    function test_snapshotMustAlreadyBeMined() public {
        ...
        r.createRaffle("x", "", uint64(block.number + 1), uint64(block.number + 20), 1, _weights());
    }

    function test_drawMustBeInTheFuture() public {
        vm.prank(owner_);
        vm.expectRevert(abi.encodeWithSelector(RaffleFacet.DrawMustBeFuture.selector, uint64(block.number)));
        r.createRaffle("x", "", uint64(block.number - 1), uint64(block.number), 1, _weights());
    }

    function test_seedCannotBeTakenEarly() public {
        uint256 id = _create();
        vm.expectRevert(abi.encodeWithSelector(RaffleFacet.DrawBlockNotReached.selector, id, uint64(1020)));
        r.anchorSeed(id);
    }
    ...
    function test_windowExpiresAndOwnerCanRearm() public {
        uint256 id = _create();
        vm.roll(1020 + 300); // past the 256-block reach of blockhash
        vm.expectRevert(abi.encodeWithSelector(RaffleFacet.DrawWindowExpired.selector, id, uint64(1020)));
        r.anchorSeed(id);
    ...
    function test_holderWithoutReaperTicketsStillGetsOne() public {
        ...
        assertEq(r.ticketsFor(id, holder, 0, 3, 0), 1);
    }

    function test_nonHolderGetsNothing() public {
        uint256 id = _create();
        assertEq(r.ticketsFor(id, stranger, 0, 0, 0), 0);
    }

    function test_consumedSoulsAddOnTop() public {
        uint256 id = _create();
        assertEq(r.ticketsFor(id, whale, 49, 348, 0), 50); // 49 consumed + 1 for holding
    }
    ...
    function test_excludedWalletCountsZeroHoweverBigItIs() public {
        ...
        assertEq(r.ticketsFor(id, whale, 49, 348, 0), 0);
    }
    ...
    function test_globalExclusionAppliesToEveryRaffle() public {
        ...
        assertEq(r.ticketsFor(id, owner_, 100, 100, 0), 0);
        uint256 id2 = _create(); // a later occasion inherits it
        assertEq(r.ticketsFor(id2, owner_, 100, 100, 0), 0);
    }
    ...
    function test_globalExclusionCanBeLifted() public {
        ...
        vm.stopPrank();
        uint256 id = _create();
        assertEq(r.ticketsFor(id, owner_, 0, 1, 0), 1);
    }

    function test_capLimitsAWhale() public {
        vm.prank(owner_);
        uint256 id = r.createRaffle("capped", "", uint64(block.number - 5), uint64(block.number + 10), 1,
            LibRaffle.Weights({perConsumedSoul: 1, perHolderWallet: 1, perSoulHeld: 0, perOGSoulHeld: 0, maxPerWallet: 10}));
        assertEq(r.ticketsFor(id, whale, 49, 348, 0), 10);
        assertEq(r.ticketsFor(id, holder, 0, 1, 0), 1); // the cap doesn't touch the small holder
    }

    function test_weightsAreFullyReconfigurable() public {
        // a later occasion that ignores reapers entirely and counts OG souls
        vm.prank(owner_);
        uint256 id = r.createRaffle("OG night", "", uint64(block.number - 5), uint64(block.number + 10), 3,
            LibRaffle.Weights({perConsumedSoul: 0, perHolderWallet: 0, perSoulHeld: 0, perOGSoulHeld: 2, maxPerWallet: 0}));
        assertEq(r.ticketsFor(id, whale, 49, 348, 244), 488); // 244 OG × 2, reapers ignored
        assertEq(r.ticketsFor(id, holder, 0, 3, 0), 0); // holds souls but no OG → out
    }
    ...
    function test_onlyOwnerConfigures() public {
        ...
        vm.expectRevert();
        r.createRaffle("x", "", uint64(block.number - 1), uint64(block.number + 5), 1, _weights());
        vm.expectRevert();
        r.setWeights(id, _weights());
        vm.expectRevert();
        r.cancelRaffle(id);
